...
try {
    ...
    try {
        ...
        Invoke-Adapter "WriteData" '{"targetRange":"Data!D1","data":["Margin"]}' | Out-Null
        if ([string]$sheet.Range("D1").Value2 -ne "Margin") {
            throw "WriteData did not verify a one-cell row-vector write"
        }
        ...
        $rangeFilter = Invoke-Adapter "FilterData" '{"range":"Data!A1:C3","column":2,"criteria":">=12","criteria2":"<=25","operator":"and"}'
        if (-not [bool]$sheet.FilterMode -or
            -not [bool]$sheet.Rows.Item(2).Hidden -or
            [bool]$sheet.Rows.Item(3).Hidden) {
            throw "FilterData did not apply the requested inclusive numeric interval"
        }
        $filterVerification = @($rangeFilter.Observation["verification"] | Where-Object { [string]$_['id'] -eq 'filter-request' })
        if ($filterVerification.Count -ne 1 -or
            [string]$filterVerification[0]['status'] -ne 'passed' -or
            [string]$filterVerification[0]['actual']['criteria1'] -ne '>=12' -or
            [string]$filterVerification[0]['actual']['criteria2'] -ne '<=25' -or
            [string]$filterVerification[0]['requestExpected']['criteria2'] -ne '<=25') {
            throw "FilterData did not return exact host-verified predicate evidence"
        }
        Invoke-Adapter "FilterData" '{"range":"Data!A1:C3","clearFilter":true}' | Out-Null
        ...
        Invoke-Adapter "WriteData" '{"targetRange":"Created!A1:B5","data":[["Metric","Value"],["Mean",1904.67],["Median",1768],["Max",3192],["Min",1000]]}' | Out-Null
        if ([string]$workbook.Worksheets.Item("Created").Range("A1").Value2 -ne "Metric" -or
            [double]$workbook.Worksheets.Item("Created").Range("B5").Value2 -ne 1000) {
            throw "WriteData could not write a rectangular Python-style result into a newly created worksheet"
        }
        ...
        $chartResult = Invoke-Adapter "CreateChart" '{"dataRange":"Data!A1:B3","type":"line","title":"Trend","position":"E3","legendPosition":"right","seriesNames":["Amount"],"categoryAxis":"Data!A2:A3"}'
        $chartProperties = @($chartResult.Observation["verification"] | ForEach-Object { [string]$_['property'] })
        foreach ($requiredProperty in @('ChartType', 'Text', 'Position', 'Name', 'XValuesHash')) {
            if ($chartProperties -notcontains $requiredProperty) {
                throw "CreateChart composite observation discarded host verification for $requiredProperty"
            }
        }
