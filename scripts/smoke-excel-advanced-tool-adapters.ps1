...
try {
    ...
    try {
        ...
        # Real model calls commonly pass structured value-field objects and a broad used range
        # polluted by unrelated output columns. The adapter must resolve requested headers,
        # select the smallest contiguous source table and preserve the requested aggregation.
        $pivotObjectSheet = $workbook.Worksheets.Add()
        $pivotObjectSheet.Name = "PivotObject"
        Invoke-Adapter "CreatePivotTable" '{"sourceRange":"Data!A1:F5","targetCell":"PivotObject!A3","rowFields":[{"field":"Category"}],"valueFields":[{"field":"Amount","aggregate":"average","caption":"Average Amount"}]}' | Out-Null
        $pivotObject = $pivotObjectSheet.PivotTables().Item(1)
        $pivotValueField = $pivotObject.DataFields(1)
        if ([int]$pivotObjectSheet.PivotTables().Count -ne 1 -or
            [string]$pivotValueField.Name -ne "Average Amount" -or
            [int]$pivotValueField.Function -ne -4106) {
            throw "CreatePivotTable did not support structured value fields over a broad observed range"
        }
