...
''' An execute label becomes host-mutation authority only when its structured Goal independently
''' passes exact-source and complete-coverage validation against the captured user request.
''' </summary>
Public NotInheritable Class IntentAcceptancePolicy
    ...
    ''' <summary>
    ''' Resolves semantic host-mutation authority without classifying the user's wording through
    ''' a command keyword list.  The model-owned execute mode is necessary but not sufficient:
    ''' its Goal must preserve and cover the exact captured request, and clarification must not
    ''' be pending.  Tool-level risk, approval and scope checks remain the SafetyGate's job.
    ''' </summary>
    Public Shared Function HasGroundedHostMutationAuthority(intent As IntentResult,
                                                            compilation As Agent.Goals.GoalCompilationResult) As Boolean
        If intent Is Nothing OrElse compilation Is Nothing Then Return False
        If ParseMode(intent.ResponseMode) <> InteractionModeKind.Execute Then Return False
        If Double.IsNaN(intent.Confidence) OrElse Double.IsInfinity(intent.Confidence) OrElse
           intent.Confidence <= MinimumAcceptedConfidence OrElse intent.Confidence > 1.0R Then
            Return False
        End If
        Return Agent.Goals.GoalCoverageValidator.Validate(compilation).Succeeded
    End Function

    ''' <summary>
    ''' A task enters the Agent when intake either selected a mandatory tool or deterministically
    ''' established that the captured request requires host execution. Compound tasks normally
    ''' have no single required future tool: selecting those tools is the adaptive loop's job.
    ''' ExpectedOutputs and ExpectedSlideCount remain completion assertions, not authority.
    ''' </summary>
    Public Shared Function HasExecutableToolRequirement(spec As Agent.AgentTaskSpec) As Boolean
        Return spec IsNot Nothing AndAlso
            (spec.RequiresHostExecution OrElse
