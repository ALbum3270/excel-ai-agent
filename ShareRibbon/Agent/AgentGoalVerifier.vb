...
Namespace Agent
    ...
    ''' to goal success: every completion-time verification requirement must match canonical
    ''' host evidence.
    ''' </summary>
    Public NotInheritable Partial Class AgentGoalVerifier
            ...
            If contract Is Nothing OrElse contract.Requirements Is Nothing OrElse
               Not contract.Requirements.Any(Function(item) item IsNot Nothing AndAlso item.Required) Then
                If String.Equals(session.AppType, "Excel", StringComparison.OrdinalIgnoreCase) Then
                    Return "任务未完成：完成决策缺少与用户目标绑定的结构化验收投影；为防止把任意局部写入误报为完成，已失败关闭。"
                End If
        ...
        Friend Shared Function TargetsOverlap(left As String, right As String) As Boolean
        ...
        Private Shared Function IsDerivedFromCapability(session As AgentSession,
            ...
            Dim isComputeProducer = validatedCapabilities IsNot Nothing AndAlso
                validatedCapabilities.Contains(capability, StringComparer.OrdinalIgnoreCase)
            Dim validatedProducers = session.Spec?.OutcomeContract?.ValidatedProducerCapabilities
            Dim isValidatedProducer = validatedProducers IsNot Nothing AndAlso
                validatedProducers.Contains(capability, StringComparer.OrdinalIgnoreCase)
            ' Persisted compute-only contracts predate ValidatedProducerCapabilities. Their
            ' compute allow-list remains trusted because it was registry-derived and sealed.
            If Not isValidatedProducer AndAlso Not isComputeProducer Then Return False

            If Not isComputeProducer Then
                ' A host tool such as DataAnalysis both performs the operation and emits the
                ' verified final-state record. It needs no transitive write lineage: the
                ' claimed evidence itself must come from that successful registered producer.
                Return String.Equals(record.SourceToolId, capability, StringComparison.OrdinalIgnoreCase) AndAlso
                    session.Iterations.Any(
                        Function(item) item IsNot Nothing AndAlso
                                       ...
                                       If(item.ContractEvidence, New List(Of OutcomeEvidenceRecord)()).Contains(record))
            End If
        ...
        Friend Shared Function ValidateContractTargetReference(appType As String,
            ...
            Dim workbookObject As String = ""
            Dim worksheetObject As String = ""
            If TryParseWorkbookWorksheetObject(decoded, workbookObject, worksheetObject) Then
                Return ""
            End If
