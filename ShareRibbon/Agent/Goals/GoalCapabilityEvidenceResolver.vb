...
Namespace Agent.Goals
    ...
    Friend NotInheritable Class GoalCapabilityEvidenceResolver
        ...
        Private Shared ReadOnly Grammars As New List(Of CapabilityGrammar) From {
            ...
            New CapabilityGrammar(
                "ExecuteVBA",
                "(?:ExecuteVBA|VBA|Visual\s+Basic\s+for\s+Applications)",
                "(?:automate|calculate|analyze|process|write|format|自动化|计算|分析|处理|写入|格式化|运行|执行)")
        }
        ...
        Friend Shared Function ClauseSupports(clauseText As String, capabilityId As String) As Boolean
            ...
            End If
            If String.Equals(If(capabilityId, "").Trim(), "ExecuteVBA", StringComparison.OrdinalIgnoreCase) Then
                Return IsExplicitVbaExecutionRequest(clauseText)
            End If
            Return False
        End Function

        Private Shared Function IsExplicitVbaExecutionRequest(input As String) As Boolean
