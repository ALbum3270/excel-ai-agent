...
Namespace Agent
    ...
    Public Class AiNativeRuntime
        ...
        Private Function BuildTaskSpec(request As AiNativeRequest,
            ...
            Dim hasPriorStructuredAuthority = priorTask IsNot Nothing AndAlso
                String.Equals(If(priorTask.MutationPolicy, ""), "allow", StringComparison.OrdinalIgnoreCase) AndAlso
                (priorTask.GoalContract IsNot Nothing OrElse priorTask.GoalCompilation IsNot Nothing)
            Dim hasStructuredAuthoritySource =
                (Not isDestinationCorrection AndAlso intent?.GoalInterpretation?.Candidate IsNot Nothing) OrElse
                hasPriorStructuredAuthority
            Dim executionAuthority = TaskExecutionAuthorityResolver.Resolve(
                intent,
                spec.GoalCompilation,
                hasStructuredAuthoritySource)
            Dim hasGroundedMutationAuthority = executionAuthority.GroundedByGoal
            Dim hasHostMutationAuthority = executionAuthority.AllowsMutation
            ' Fail closed at task intake. AgentTaskSpec defaults to "allow" for historical
            ' callers, so this runtime must always overwrite it from the single authority
            ' decision before any routing or tool projection occurs.
            spec.MutationPolicy = If(hasHostMutationAuthority, "allow", "read_only")
            Dim isReadOnlyDataAnswer = IsReadOnlyExcelDataAnswer(
                intent,
                request.OfficeContext,
                appType,
                hasHostMutationAuthority)
            Dim isNonExecution = Not isReadOnlyDataAnswer AndAlso
                Not hasHostMutationAuthority
            ...
            AppLogger.Info(
                "AiNativeRuntime",
                $"Task execution authority appType={appType} mode={interactionMode} source={executionAuthority.Source} mutation={hasHostMutationAuthority} goalFallback={Not String.IsNullOrWhiteSpace(spec.GoalInterpretationFallbackReason)}")
            ' Routing must not wait for a future tool choice. A compound Office request often
            ' has no single RequiredTool at intake; the adaptive loop chooses one action only
            ' after observing the current workbook and every previous tool result.
            spec.RequiresHostExecution = isReadOnlyDataAnswer OrElse hasHostMutationAuthority
            ...
            If isNonExecution Then
                ' Ordinary answers and clarifications now run through the same adaptive Agent
                ' as tool-backed work. They retain a deterministic no-mutation boundary.
            ...
            Else
                ...
                If hasGroundedMutationAuthority Then
                    spec.Constraints.Add("宿主写入授权来自已通过逐字来源与完整覆盖校验的结构化 Goal；每个具体动作仍须通过工具权限和 SafetyGate")
                End If
            End If
            ...
            ' RequiredTools is a semantic/planning projection, not a copy of a retrieved
            ' Skill's handbook. Built-in host capability is authorized independently by the
            ' application baseline; a specialised Skill may advise strategy but must not
            ' shrink that capability universe. Only deterministic method constraints and
            ' selected Skill-private scripts belong in this projection.
            ...
            If isReadOnlyDataAnswer Then
                ...
                If isExplicitPythonCompute Then
        ...
        Private Shared Sub AttachGoalInterpretation(spec As AgentTaskSpec,
            ...
            Dim compilation As Goals.GoalCompilationResult = Nothing
            ...
            If Not isDestinationCorrection AndAlso intent?.GoalInterpretation?.Candidate IsNot Nothing Then
                Try
                    Dim adapter As New Goals.ModelGoalInterpretationAdapter(intent.GoalInterpretation)
                    compilation = adapter.Interpret(authoritativeRawRequest)
                Catch ex As Exception
                    fallbackReason = "Structured goal interpretation could not be compiled and exact user text was preserved: " & ex.Message
                End Try
            Else
                fallbackReason = If(isDestinationCorrection,
            ...
            If compilation IsNot Nothing Then
                ...
                If Not validation.Succeeded AndAlso Not compilation.RequiresClarification Then
                    fallbackReason = "Structured interpretation rejected by deterministic validation: " & String.Join("; ", validation.Errors)
                    compilation = Nothing
                End If
            End If

            If compilation Is Nothing Then
                If String.IsNullOrWhiteSpace(fallbackReason) Then
                    fallbackReason = "No validated structured Goal was available; exact user text was preserved."
                End If
                spec.RecordGoalInterpretationFallback(fallbackReason)
                Dim fallback As New Goals.RawPreservingGoalInterpretationAdapter(fallbackReason)
                compilation = fallback.Interpret(authoritativeRawRequest)
                Dim fallbackValidation = Goals.GoalCoverageValidator.Validate(compilation)
                If Not fallbackValidation.Succeeded Then
                    Throw New InvalidOperationException("Raw-preserving Goal fallback failed validation: " & String.Join("; ", fallbackValidation.Errors))
                End If
            End If

            spec.SetGoalCompilationOnce(compilation)
        End Sub

        Private Shared Function IsReadOnlyExcelDataAnswer(intent As IntentResult,
                                                           officeContext As Context.OfficeContext,
                                                           appType As String,
                                                           hasHostMutationAuthority As Boolean) As Boolean
            ...
            ' A naked model mode is not authority. TaskExecutionAuthorityResolver accepts it
            ' only when the structured Goal preserves and covers the captured request.
            Dim isDataAnalysis = intent.OfficeIntent = OfficeIntentType.DATA_ANALYSIS OrElse
                intent.IntentType = ExcelIntentType.DATA_ANALYSIS
            ' The structured Goal payload is a stronger signal than the legacy enum. The
            ' intent prompt requires it for any answer that must inspect Office state, even
            ' when a provider incorrectly labels the request GENERAL_QUERY. Pure chat/help
            ' answers keep GoalInterpretation=null and remain on the conversational path.
            Dim hasStructuredWorkbookGoal = intent.GoalInterpretation?.Candidate IsNot Nothing
            isDataAnalysis = isDataAnalysis OrElse hasStructuredWorkbookGoal
            Return isDataAnalysis AndAlso Not hasHostMutationAuthority
        End Function
