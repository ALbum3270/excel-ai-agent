    ...
    Public Partial Class LoopEngine
        ...
        Public Async Function RunAsync(session As AgentSession,
            ...
            Try
                ...
                ' Mutation authority is a trusted UI control, independent from model planning.
                ' A model may choose not to write, but it cannot grant itself write access and
                ' an execution request cannot silently collapse back to read-only routing.
                session.Spec.MutationPolicy = If(
                    String.Equals(session.RequestedMutationPolicy, "allow_mutation", StringComparison.OrdinalIgnoreCase),
                    "allow_mutation",
                    "read_only")
