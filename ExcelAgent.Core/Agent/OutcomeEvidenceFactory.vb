...
Namespace Agent
    ...
    Public NotInheritable Class OutcomeEvidenceFactory
        ...
        Public Shared Function Create(tool As ToolDescriptor,
            ...
            Dim dataHash = OutcomeEvidenceValueHasher.Compute(result.Data)
        ...
        Private Shared Function ResolveAtomicEffect(declaredEffects As List(Of String),
            ...
            ' A dedicated lifecycle tool may verify an implementation property such as Name
            ' while its public semantic effect is object_exists. If the host did not emit a
            ' typed effect, the sole declared effect remains the safe fallback. Once the host
            ' does emit a type, however, preserve it only when the tool explicitly declares
            ' that it can prove that type; never relabel property evidence as an artifact.
            If declaredEffects.Count = 1 AndAlso String.IsNullOrWhiteSpace(itemEffect) AndAlso
               String.IsNullOrWhiteSpace(observedEffect) Then Return declaredEffects(0)
            Return ""
        End Function
        ...
        Private Shared Sub BindReferenceProperty(container As JObject,
            ...
            Dim referenceToken = container(propertyName)
            If referenceToken Is Nothing OrElse referenceToken.Type <> JTokenType.String Then Return
        ...
        Private Shared Function ExtractVerifiedPropertyName(observation As JObject,
            ...
            If verification Is Nothing Then
                Dim verificationObject = TryCast(observation?("verification"), JObject)
                If VerificationObjectApplies(verificationObject, targetRef, resolvedTargetCount) Then
                    Return If(verificationObject?("property")?.ToString(), "").Trim()
                End If
                Return ""
            End If
