...
Namespace Agent
    ...
    Public NotInheritable Partial Class AgentGoalVerifier
        ...
        Private Shared Function CanonicalObjectIdentity(value As String) As String
            ...
            Dim shorthandWorkbook As String = ""
            Dim shorthandWorksheet As String = ""
            If TryParseWorkbookWorksheetObject(decoded, shorthandWorkbook, shorthandWorksheet) Then
                Return BuildWorksheetIdentity(shorthandWorkbook, shorthandWorksheet)
            End If
        ...
        Friend Shared Function BindActiveWorkbookReference(value As String,
            ...
            ' Some model providers emit Workbook.xlsx!Worksheet for a worksheet lifecycle
            ' target.  That is an object identity, not Sheet!A1 range syntax.  Normalize it
            ' at the shared ObjectRef seam so planning and host evidence use one identity.
            Dim explicitWorkbook As String = ""
            Dim explicitWorksheet As String = ""
            If TryParseWorkbookWorksheetObject(source, explicitWorkbook, explicitWorksheet) Then
                Return $"Excel:workbooks/{Uri.EscapeDataString(explicitWorkbook)}/worksheets/{Uri.EscapeDataString(explicitWorksheet)}"
            End If

            If String.IsNullOrWhiteSpace(workbook) Then Return source
        ...
        ''' <summary>
        ''' Accepts the legacy workbook-qualified worksheet shorthand without confusing it
        ''' with the established Worksheet!A1 range form.  Requiring a workbook file suffix
        ''' keeps ambiguous Sheet!ChildObject strings fail-closed.
        ''' </summary>
        Private Shared Function TryParseWorkbookWorksheetObject(value As String,
                                                                 ByRef workbook As String,
                                                                 ByRef worksheet As String) As Boolean
            workbook = ""
            worksheet = ""
            ...
            Dim match = Regex.Match(
                decoded,
                "^(?:Excel:)?(?<workbook>[^!/:]+\.(?:xlsx?|xlsm|xlsb|csv))!(?<worksheet>[^!/:]+)$",
                RegexOptions.IgnoreCase)
            If Not match.Success Then Return False

            workbook = match.Groups("workbook").Value.Trim("'"c)
            worksheet = match.Groups("worksheet").Value.Trim("'"c)
            Return Not String.IsNullOrWhiteSpace(workbook) AndAlso
                Not String.IsNullOrWhiteSpace(worksheet)
        End Function
