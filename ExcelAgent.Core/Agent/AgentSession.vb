...
Namespace Agent
    ...
    Public Class AgentSession
        ...
        ''' <summary>
        ''' Mutation authority comes from an explicit host UI control, never from keyword
        ''' routing or a model label. The default remains fail-closed for non-UI callers.
        ''' </summary>
        Public Property RequestedMutationPolicy As String = "read_only"
    ...
    Public Class AgentTaskSpec
        ...
        Public Property Complexity As String = "medium"
        ''' <summary>
        ''' Controls whether the task may mutate the active Office document. It is fail-closed;
        ''' only trusted intake may replace the default after validating execution authority.
        ''' </summary>
        Public Property MutationPolicy As String = "read_only"
        ''' <summary>
        ''' Deterministic routing fact derived from the captured request and observable host
        ''' context. Unlike ResponseMode, this is not granted by a model label. It tells the
        ''' chat router that the goal must enter the adaptive Agent/tool loop even when no
        ''' particular future tool has been selected yet.
        ''' </summary>
        Public Property RequiresHostExecution As Boolean = False
        ''' <summary>
    ...
    Public Class OutcomeContract
        ...
        ''' when a completion-time verification projection is sealed.
        ''' </summary>
        ...
        ''' <summary>
        ''' Registered capabilities that directly produced the requirement's observed effect.
        ''' Compute capabilities are also producers, but their persisted output additionally
        ''' requires dependency-lineage verification through the later write action.
        ''' </summary>
        <Newtonsoft.Json.JsonIgnore>
        Public Property ValidatedProducerCapabilities As New List(Of String)()
