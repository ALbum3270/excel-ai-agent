...
2. Use only registered JSON tools. There is no VBA fallback; for long-tail object-model work use `DiscoverOfficeCapability` followed by `OfficeObjectOperation`.
...
13. Choose a source by semantic granularity, not merely because it is the active sheet. For requests about original records, inspect the workbook worksheet index and prefer a table whose headers and row granularity match the requested fields. An aggregate/report/output sheet is not a valid substitute for row-level source data; use `ReadRange` to observe the best matching candidate before computing.
14. Every mutating `OfficeObjectOperation` must include an observable `expectedEffects` object or batch `successCriteria`. A host call returning without an exception is not proof of success.
